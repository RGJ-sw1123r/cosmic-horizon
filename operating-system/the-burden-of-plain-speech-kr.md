...
이 질문들이 지시 안에 고정되어 있지 않으면 AI는 스스로 답의 형태를 만든다.
...
프롬프트를 잘 쓴다는 것은 기계 앞에 말을 많이 쏟아 붓는 기술보다, AI가 다르게 해석해서는 곤란한 경계선을 고정하는 일에 가깝다.
...
처음부터 단순해 보이는 말은 대개 정제된 단순함이 아니라 누락에 가깝다.
...
그리고 다른 맥락으로 이식할 수 있는 생각이다.

확장된 생각은 다른 사람이 실행해도 원래 방향을 보존하는 생각이다.

AI에게 한 번 던지면 사라지는 소모품이 아니라, 엔지니어링을 위한 영속적인 좌표가 된다.
