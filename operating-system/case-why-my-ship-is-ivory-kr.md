...
디자이너와 개발자는 이제 빈 화면이 아니라, 보정 가능한 초안을 앞에 두고 대화할 수 있었다.
...
이 흐름에서 개발자의 역할은 퍼블리셔가 넘긴 정적 화면 결과물을 제품 구조에 연결하는 후속 담당자에서, 디자인 맥락과 AI 생성 코드, 제품 아키텍처 사이의 차이를 검산하고 조정하는 위치로 이동한다.
...
이 지점에서 웹 퍼블리셔의 역할은 남거나, 다른 방향으로 재편될 수 있다.
