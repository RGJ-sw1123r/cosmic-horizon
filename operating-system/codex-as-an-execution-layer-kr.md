...
* 도구는 항해의 목적에 맞게 선택되어야 한다.
* AI-assisted development의 핵심은 사람이 판단할 영역과 AI가 실행할 영역을 나누는 능력이다.
...
* 핵심은 웹 기획 공간과 CLI 실행 공간의 분리다.
* 이 문서는 ChatGPT를 기획 공간으로, Codex를 실행 계층으로 배치하는 운영 방식을 설명한다.
* AGENTS.md와 agent instruction file은 AI agent를 선언된 경계 안에서 움직이게 만드는 운영 장치다.
* 도구의 비용 구조와 작업 흐름은 작업자가 생각하고 실행하는 방식을 바꾸는 운영 조건이다.

## 좋은 도구의 기준

좋은 도구는 유행보다, 지금 나서는 항해에 맞는지로 판별된다.
...
ChatGPT는 AI를 본격적인 작업 도구로 받아들이게 만든 첫 번째 인터페이스였다. 2023년 무렵부터 ChatGPT는 사고, 초안 작성, 검토, 개발 보조, 문서화에 계속 사용해 왔다.

오래 사용한 만큼 익숙하고, 정이 든 것도 사실이다.

하지만 최종 선택은 익숙함보다 운영 구조에서 결정되었다.
...
사람이 더 잘하는 일이 있고, AI가 더 빠르게 처리할 수 있는 일이 있다.
...
* 완료 기준을 정의하는 일.
* AI가 추론할 수 있는 영역과 추론 금지 영역을 나누는 일.
...
AI 활용 개발에서는 문서화가 실행보다 앞선다.

이 운영체계에서 문서는 인간의 판단을 AI가 따라갈 수 있는 좌표로 바꾸는 계층이다.

좋은 지시는 단순 요청을 넘어 설계된 artifact가 된다.

목표가 무엇인지, 어떤 맥락을 보존해야 하는지, 어떤 제약을 지켜야 하는지, 무엇이 완료 상태인지가 문서 안에 명시되어야 한다.

OpenAI의 Codex best practices도 좋은 작업 지시에 Goal, Context, Constraints, Done when을 넣으라고 설명한다.

이 기준은 AI가 추론만으로 빈칸을 채워야 하는 여지를 줄이는 엔지니어링이다.
...
이 작업은 선언된 프레임 안에서 실행될 때 가치를 갖는다.
...
하지만 검토 비용, 되돌리기 비용, 책임 비용은 뒤에서 눈덩이처럼 불어난다.
...
도구는 작업자의 사고를 유지하고 확장해야 한다.

좋은 항법은 상위 계층에서 충분히 관측하고 판단한 뒤, 항해를 안전하고 빠르게 만든다.
...
이 통계 수치는 이 항법체계를 직접 증명하기보다 경계를 보여준다.
...
그리고 이 조사는 개발자가 고객 요구, UX 판단, 제약 조건, 완료 기준을 먼저 문서화한 뒤 AI agent를 하위 실행 계층으로 호출하는 사용법까지 세밀하게 다루기 전 단계에 있다.

바로 그 지점에 이 문서의 좌표가 있다.

인간의 판단을 상위에서 명시하고, AI 실행을 하위에서 통제하는 방식은 아직 일반적인 설문 결과에 포함되어 있지 않다.

이 문서는 통계 안에서 아직 이름이 붙기 전의 사용법을 기록하려는 시도이다.

## 통계가 말하지 않는 사용법

Stack Overflow 조사는 또 다른 경계도 보여준다.

개발자들은 AI를 검색, 학습, 코드 작성, 문서화, 테스트 같은 영역에는 비교적 쉽게 도입한다. 반면 배포와 모니터링에는 76%, 프로젝트 계획에는 69%가 AI를 사용할 계획이 없다고 답했다.

이 경계는 합리적이다.

프로젝트 계획, 고객 요구 해석, 화면 목적 정의, UX 판단은 사람이 책임져야 할 상위 판단이다.

동시에 AI는 이 영역에서 관측 보조자, 질문자, 정리자, 검산자, 실행 지시 정제자가 될 수 있다.

프로젝트 계획을 AI에게 위임하는 방식과, 사람이 프로젝트 계획을 더 잘 책임질 수 있도록 AI로 문서화를 강화하는 방식은 서로 다르다.

책임 위임과 책임 선명화는 서로 다른 작업 방식이다.

이 운영체계는 후자를 선택한다.

## 도구 선택은 운영 결정이다

도구 선택은 브랜드 선호보다 운영 결정에 가깝다.

도구는 운영체계의 일부다.

따라서 이 문서는 현재 작업 방식에서 실행 계층에 놓일 도구를 판별하는 기준을 다룬다.

핵심은 제품의 우열보다, 기획 공간과 실행 계층의 분리를 어떤 도구가 가장 잘 보여주는가에 있다.

AGENTS.md나 agent instruction file도 같은 기준에서 보아야 한다.

그 원칙은 Codex 전용 문법에 갇히지 않는다. Claude Code, Gemini CLI, Antigravity, Codex처럼 프로젝트 문맥을 읽고 실행하는 AI agent에게 모두 적용될 수 있는 운영 경계다.

벤더는 달라질 수 있다.

하지만 agent가 어디를 봐야 하는지, 언제 움직여야 하는지, 추론 금지 영역이 어디인지, 어디서 멈춰야 하는지를 선언해야 한다는 원칙은 유지된다.
...
상위의 사고 단계는 하위의 실패를 줄이는 투자다.

문서화 단계는 agent가 나중에 저지를 실수를 태어나기 전에 줄이는 곳이다.

검증 기준을 먼저 선언하는 일은 인간의 책임을 보이게 만드는 일이다.

이 단계를 비싸게 만들거나 취약하게 만드는 도구는 이 운영 모델과의 적합성이 낮다.

## 시장 신호로서의 Codex

Codex에도 플랜별 사용 한도가 있다.

한도에 가까워지거나 도달하면 플랜에 따라 크레딧 추가, 업그레이드, reset까지 대기 같은 선택지가 필요할 수 있다.

하지만 동시에 시장에서 드러난 신호도 함께 봐야 한다.
...
이 발언은 제품 방향을 보여주는 시장 신호로 읽을 수 있다.

적어도 그 순간 OpenAI는 Codex를 ChatGPT 플랜 안에서 넓게 제공하려는 방향을 공개적으로 보여주었다.
...
이 운영체계의 중심은 Codex 실행 전에 존재하는 관측과 판단이다.

ChatGPT는 상위의 관측, 질문, 정제, 문서화를 돕는다.

Codex는 하위의 구현, 수정, 실행, 검증을 돕는다.

둘 사이의 분리는 비용 문제이자 운영 문제다.

AI가 더 빠르게 구현할수록, 인간은 더 선명하게 관측해야 한다.

## 결론 (Conclusion)

OpenAI Codex는 함장이 정한 항로를 기계실에서 밀어주는 실행 엔진이다.

Codex는 관측, 판단, 문서화 아래에 설치되는 실행 계층이다.

이 운영체계에서 인간의 일은 AI가 움직일 수 있는 좌표를 책임 있게 선언하는 것이다.
