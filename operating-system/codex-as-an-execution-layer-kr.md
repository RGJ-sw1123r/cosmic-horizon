...
핵심은 제품의 우열이 아니라, 기획 공간과 실행 계층의 분리를 어떤 도구가 가장 잘 보여주는가에 있다.
...
* [\[pattern\] FTL-Bound Agents (KR)](pattern-ftl-bound-agents-kr/)는 Codex 같은 실행 계층이 해석을 통해 작업 범위를 넓히지 않도록, agent instruction file을 운영 경계로 다루는 방식을 설명한다.
* [\[protocol\] AGENTS.md Blueprint (KR)](pattern-ftl-bound-agents-kr/protocol-agents.md-blueprint-kr.md)는 그 경계를 AGENTS.md 형태의 반복 가능한 프로젝트 지시 체계로 구현하는 방식을 보여준다.
